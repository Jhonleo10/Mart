...
function staticSitemapEntries(baseUrl: string): MetadataRoute.Sitemap {
  return [
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticRoutes = staticSitemapEntries(baseUrl);

  const dynamicRoutes = await safeDbQuery(
    "sitemap",
    async () => {
      ...
      return [
        ...products.map((p) => ({
          url: `${baseUrl}${getProductPublicPath(p.slug)}`,
          lastModified: p.landingPage?.publishedAt ?? p.updatedAt,
